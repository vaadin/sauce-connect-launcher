...
var
  ...
  SAUCE_CONNECT_VERSION = process.env["SAUCE_CONNECT_VERSION"] || "5";
...
function getArchiveName(downloadInfo) {
  return downloadInfo.url.split("/").pop();
}

function getFolderName(downloadInfo) {
  return getArchiveName(downloadInfo).replace(/\.zip$/, "").replace(/\.tar\.gz$/, "");
}

function getScBin(downloadInfo) {
  var exe = process.platform.startsWith("win") ? "sauce-connect.exe" : "sc";
  return path.normalize(scDir + "/" + getFolderName(downloadInfo) + "/" + exe);
}
...
function unpackArchive(archivefile, archiveFolder, callback) {
  ...
  setTimeout(function () {
    // Create archive folder beforehand, as Sauce Connect 5 archives don't contain a folder anymore
    if (!exists(archiveFolder)) {
      fs.mkdirSync(archiveFolder);
    }

    if (archivefile.match(/\.tar\.gz$/)) {
      exec("tar -xzf '" + archivefile + "'", {cwd: archiveFolder}, done);
    } else {
      try {
...
function verifyChecksum(downloadInfo, archivefile, cb) {
  var fd = fs.createReadStream(archivefile);
  var checksum = downloadInfo.checksums[0];
  var hash = crypto.createHash(checksum.algorithm);
  hash.setEncoding("hex");

  hash.on("finish", function() {
    var sha1 = hash.read();
    if (sha1 !== checksum.value) {
      return cb(new Error("Checksum of the downloaded archive (" + sha1 + ") doesn't match (" + checksum.value + ")."));
    }
...
function fetchArchive(downloadInfo, archivefile, callback) {
  ...
  var req = httpsRequest({
    host: SAUCE_API_HOST,
    port: 443,
    path: downloadInfo.url.replace("https://" + SAUCE_API_HOST, ""),
  });
...
function fetchAndCheckArchive(downloadInfo, archivefile, callback) {
  return async.waterfall([
    async.apply(fetchArchive, downloadInfo, archivefile),
    async.apply(verifyChecksum, downloadInfo, archivefile)
  ], callback);
}

function unpackAndFixArchive(archivefile, archiveFolder, bin, callback) {
  return async.waterfall([
    async.apply(unpackArchive, archivefile, archiveFolder),
    async.apply(setExecutePermissions, bin)
  ], callback);
}

function fetchAndUnpackArchive(downloadInfo, options, callback) {
  var bin = getScBin(downloadInfo);
  if (exists(bin)) {
    return callback(null, bin);
  }

  var archivefile = path.normalize(scDir + "/" + getArchiveName(downloadInfo));
  var archiveFolder = path.normalize(scDir + "/" + getFolderName(downloadInfo));
  ...
  async.waterfall([
    async.apply(fetchAndCheckArchive, downloadInfo, archivefile),
    async.apply(unpackAndFixArchive, archivefile, archiveFolder, bin)
  ], callback);
}
...
function getDownloadInfo(options, cb) {
  var os = getPlatform();
  var arch = getCPUArch();
  var query = `?os=${os}&arch=${arch}&version=${SAUCE_CONNECT_VERSION}`;

  var req = httpsRequest({
    host: SAUCE_API_HOST,
    port: 443,
    path: "/rest/v1/public/tunnels/sauce-connect/download" + query
  });
  ...
  req.on("response", function (res) {
    if (res.statusCode !== 200) {
      ...
      return cb(new Error("Fetching Sauce Connect download info failed: " + res.statusCode));
    }
    ...
    res.on("end", function () {
      ...
      var responseJson = buffer.toString('utf8');
      var responseData = JSON.parse(responseJson);
      var download = responseData["download"];
      req.destroy();
      cb(null, download);
    });
  });
...
function download(options, callback) {
  ...
  async.waterfall([
    async.apply(getDownloadInfo, options),
    function (downloadInfo, next) {
      return fetchAndUnpackArchive(downloadInfo, options, next);
    }
  ], callback);
}
...
function connect(bin, options, callback) {
  ...
  var args = processOptions(options),
    ...
    dataActions = {
      ...
      "Sauce Connect is up, you may start your tests": ready,
  ...
  /* Fix args to work with Sauce Connect 5 */

  // sc binary now requires a command
  args.unshift("run");

  // Region is required now
  args.push("--region", "us-west");

  // Map tunnel-identifier argument to tunnel-name
  args = args.map((arg) => {
    if (arg === "--tunnel-identifier") {
      return "--tunnel-name";
    }
    return arg;
  });

  // Remove log-file argument, it's not supported (anymore?)
  var logFileIdx = args.findIndex((arg) => arg === "--logfile");
  if (logFileIdx !== -1) {
    args.splice(logFileIdx, 2);
  }
