...
function getScFolderName(version) {
  return {
    darwin: "sauce-connect-" + version + "_darwin.all",
    win32: "sauce-connect-" + version + "_windows.x86_64"
  }[process.platform] || "sauce-connect-" + version + "_linux.x86_64";
}
...
function getScBin(version) {
  ...
  return path.normalize(scDir + "/" + getScFolderName(version) + "/sc" + exe);
}
...
function unpackArchive(archivefile, callback) {
  ...
  setTimeout(function () {
    ...
    var archiveFolder = archivefile.replace(/\.zip$/, "").replace(/\.tar\.gz$/, "");
...
function fetchArchive(version, archiveName, archivefile, callback) {
  ...
  var req = httpsRequest({
    host: SAUCE_API_HOST,
    port: 443,
    path: "/downloads/sauce-connect/" + version + "/" + archiveName
  });
...
function fetchAndCheckArchive(version, archiveName, archivefile, checksum, callback) {
  return async.waterfall([
    async.apply(fetchArchive, version, archiveName, archivefile),
...
function fetchAndUnpackArchive(versionDetails, options, callback) {
  ...
  async.waterfall([
    async.apply(fetchAndCheckArchive, versionDetails.version, archiveName, archivefile, versionDetails.checksum),
...
function getVersion(options, cb) {
  ...
  // Below is the old code that fetches the versions.json file from Sauce Labs
  // Since that file hasn't been updated for Sauce Connect 5 yet, we're using a
  // local copy of versions.json for now, which contains download links for
  // Sauce Connect 5 in the same format that the old versions.json file did
  fs.cpSync(path.normalize(__dirname + "/versions.v5.json"), versionsfile);
